...
interface IHighPotentialResponse {
...
interface IHighPotentialProducts {
  id: string;
  name: string;
  price: number;
  sku: string;
...
const highPotential = async (
  ...
  if (!workspaceId || !environmentId || !startDate || !endDate) {
    ...
    const highPotentialProducts: IHighPotentialProducts[] = [];

    const response = await apiClient.get<
      string,
      { response: IHighPotentialResponse[] }
    >(`${APIS_ROUTES.HIGH_POTENTIAL_PRODUCTS}`, {
      params: {
        date_range: 'CUSTOM',
    ...
    if (skuIds.length) {
      ...
      if (response.response.Data.items.length > 0) {
        ...
        response.response.Data.items.forEach((image: any, index: number) => {
          ...
          highPotentialProducts.push(items);
        });
      }
    }

    return highPotentialProducts;
  }
};

const useHighPotential = (
  workspaceId: string,
  environmentId: string | null,
  startDate?: string,
  endDate?: string
  ...
  useQuery(
    [
      API_QUERY_KEY.HIGH_POTENTIAL,
      workspaceId,
      environmentId,
      startDate,
      endDate,
    ],
    () => highPotential(workspaceId, environmentId, startDate, endDate)
  );

export default useHighPotential;
