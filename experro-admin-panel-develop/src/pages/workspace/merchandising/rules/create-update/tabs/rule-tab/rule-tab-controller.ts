...
const useRuleTabController = ({
  ...
  const onFinish = (rule: RulesEj) => {
    if (rule.name?.trim().length === 0) {
      ...
      if (isSliderVisible && rule.strength) {
        if (rule.strength > 0) {
          rule.operation = 'boost';
        } else {
          rule.operation = 'bury';
          rule.strength = Math.abs(rule.strength);
        }
      }
      // console.log(rule);
  ...
  const onDuplicateSubRule = (rule: RulesEj) => {
    ...
    if (rule.operation === 'bury' && rule.strength) {
      clonedRule.strength = -Math.abs(rule.strength);
    }
    ...
    if (rule.operation === 'include' || rule.operation === 'exclude') {
      ...
      clonedRule.operation = 'boost/bury';
    }
  ...
  const onEditSubRule = async (rule: RulesEj) => {
    ...
    if (rule.operation === 'bury' && rule.strength) {
      editedRule.strength = -Math.abs(rule.strength);
    }
    ...
    if (rule.operation === 'include' || rule.operation === 'exclude') {
      ...
      setIsSliderVisible(true);
      editedRule.operation = 'boost/bury';
    }
  ...
  const onAddRule = () => {
    setIsEditing(true);
    setAddRuleItem(true);
    setIsAddButtonVisibleRule(true);
    form.resetFields();
  };
