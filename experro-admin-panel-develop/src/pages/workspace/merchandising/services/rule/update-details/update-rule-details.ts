...
const updateRuleDetails = async (
  ...
  if (!workspaceId && !contentModelId && !contentModelDataId) {
    ...
    if (
      ...
      data.dynamicFieldsData.startDateEdti !== 'Invalid date'
    ) {
      data.dynamicFieldsData.startDateEdti = new Date(
        data.dynamicFieldsData.startDateEdti
      ).toISOString();
    } else {
      if (data.dynamicFieldsData.startDateEdti === 'Invalid date') {
        data.dynamicFieldsData.startDateEdti = '';
      }
    }
    ...
    if (
      ...
      data.dynamicFieldsData.endDateEdti !== 'Invalid date'
    ) {
      data.dynamicFieldsData.endDateEdti = new Date(
        data.dynamicFieldsData.endDateEdti
      ).toISOString();
    } else {
      if (data.dynamicFieldsData.endDateEdti === 'Invalid date') {
        delete data.dynamicFieldsData.endDateEdti;
      }
    }
