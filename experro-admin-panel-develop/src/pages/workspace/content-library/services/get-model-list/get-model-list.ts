...
const modelList = async (workspaceId: string, envId: string | null) => {
  ...
  >(`${APIS_ROUTES.CONTENT_SERVICE}/${workspaceId}/content-models`, {
    params: {
      fields:
        'name,type,created_at,template,act_as_web_page,internal_name,is_localization_enabled,group_id,description,template,environment_id,edge_caching,is_versionable,is_system_created',
...
const useGetModelList = (workspaceId: string, envId: string | null) =>
  useQuery<ContentModelList[], IAPIError>(
    [API_QUERY_KEY.CONTENT_LIBRARY_CONTENT_MODEL_LIST],
