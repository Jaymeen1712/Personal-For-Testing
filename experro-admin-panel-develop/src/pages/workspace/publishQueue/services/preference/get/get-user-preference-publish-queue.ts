...
const getUserPreferencePublishQueue = async (
  workspaceId: string,
...
const useGetUserPreferencePublishQueue = (
  workspaceId: string,
  preferenceName: string,
  userId?: string
) =>
  useQuery<{ preferenceValue: string[]; id: string }[], IAPIError>(
    ...
    () => getUserPreferencePublishQueue(workspaceId, preferenceName, userId)
  );

export default useGetUserPreferencePublishQueue;
