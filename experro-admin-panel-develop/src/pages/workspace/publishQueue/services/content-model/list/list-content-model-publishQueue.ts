...
const listContentModelPublishQueue = async (
  workspaceId: string,
  envId: string | null
) => {
  ...
  >(`${APIS_ROUTES.CONTENT_SERVICE}/${workspaceId}/public-contents`, {
    params: {
      fields:
...
const useListContentModelPublishQueue = (
  workspaceId: string,
  envId: string | null
) =>
  useQuery<ContentModelList[], IAPIError>(
    [API_QUERY_KEY.CONTENT_MODEL_LIST],
    () => listContentModelPublishQueue(workspaceId, envId),
    {
      cacheTime: 0,
    }
  );

export default useListContentModelPublishQueue;
