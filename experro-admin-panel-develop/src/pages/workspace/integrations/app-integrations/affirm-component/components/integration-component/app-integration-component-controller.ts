...
const useIntegrationComponentController = () => {
  ...
  const onAcceptButtonClick = () => {
    appInstall.mutate({
      isActivated: true,
      integrationInternalName: 'affirm_marketing',
  ...
  const onCancelButtonClick = () => {
    history.push(
      `/workspaces/${workspaceId}/apps/affirm_marketing/${id}/${categoryId}`
    );
  };
  ...
  useEffect(() => {
    if (appInstall.isSuccess) {
      ...
      if (appInstall.data) {
        history.push(
          `/workspaces/${workspaceId}/apps/affirm_marketing/${id}/${categoryId}/installed`
        );
      }
    }
