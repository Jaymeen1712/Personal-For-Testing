...
const useIntegrationComponentController = () => {
  ...
  const onAcceptButtonClick = () => {
    appInstall.mutate({
      isActivated: true,
      integrationInternalName: 'algolia',
      masterIntegrationId: id,
      categoryId,
    });
  };
  const onCancelButtonClick = () => {
    history.push(`/workspaces/${workspaceId}/apps/algolia/${id}/${categoryId}`);
  };
  ...
  useEffect(() => {
    if (appInstall.isSuccess) {
      ...
      if (appInstall.data) {
        history.push(
          `/workspaces/${workspaceId}/apps/algolia/${id}/${categoryId}/installed`
        );
      }
    }
