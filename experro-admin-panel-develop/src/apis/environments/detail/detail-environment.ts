...
import { IAPIError, IAxiosResponse, IGetEnvironment } from '../../../types';
...
const detailsEnvironment = async (
  ...
  const response = await apiClient.get<string, IAxiosResponse<IGetEnvironment>>(
    `${APIS_ROUTES.ENVIRONMENT_SERVICE}/${workspaceId}/environments/${environmentId}`,
    {
      params: {
        fieldsToQuery:
          'title,workspace_domain,custom_domain,enable_password_protect,enable_maintenance,password_hash,http_robots,type,point_your_domain,site_map_url',
      },
    }
  );
