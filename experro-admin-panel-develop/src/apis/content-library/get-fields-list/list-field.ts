...
import { FormFieldValues, IAPIError, IAxiosResponse } from '../../../types';
...
const listField = async (
  ...
  >(
    `${APIS_ROUTES.CONTENT_SERVICE}/${workspaceId}${APIS_ROUTES.CONTENTS}/${contentId}${APIS_ROUTES.CONTENT_FIELDS}`,
    {
      params: {
        fields:
          'field_properties,is_required,type,title,id,field_name,relation_type,position,destination_content_model_id,is_editable,is_data_editable',
      },
    }
  );
